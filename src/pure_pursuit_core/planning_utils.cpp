...
std::vector<geometry_msgs::Pose> extractPoses(const nav_msgs::Path::ConstPtr & path)
{
  ...
  double th_dist, double th_yaw)
{
  ...
  for (size_t i = 0; i < poses.size(); ++i) {
    ...
    if (ds > th_dist * th_dist) {
      // ROS_INFO("waypoint:[%f, %f]", poses.at(i).position.x, poses.at(i).position.y);
      // ROS_INFO("curr pose:[%f, %f]", current_pose.position.x, current_pose.position.y);
      // ROS_INFO("ds2: %f, th2: %f", ds, th_dist * th_dist);
      continue;
    }
    ...
    // ROS_WARN("ds2: %f, th2: %f", ds, th_dist * th_dist);
...
int8_t getLaneDirection(const std::vector<geometry_msgs::Pose> & poses, double th_dist)
{
  ...
  for (uint32_t i = 0; i < poses.size(); i++) {
    ...
    // double dist2 = planning_utils::calcDistSquared2D(prev.position, next.position);
    // ROS_WARN("prev(%f, %f), next(%f, %f), dist2 : %f", prev.position.x, prev.position.y, next.position.x, next.position.y, dist2);
